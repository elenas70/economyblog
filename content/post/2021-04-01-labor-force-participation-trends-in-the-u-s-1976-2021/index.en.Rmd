...
author: "[Elena Stolpovsky](https://www.linkedin.com/in/estolpovsky/)"
date: '2021-04-01'
slug: labor-force-participation-trends-in-the-u-s-1976-2021
categories:
  - Labor Market
tags:
  - COVID
...
```
*Source: Current Population Survey, 1976–2020*
...
During 1976⁠–2020 the rates of being out of work due to disability increased from 2.5 to 5.3% for working-age men and from 0.7 to 5% for working-age women. The peaks in disability rates occurred in 2015 at 6.3% for both men and women.The rising disability rates contributed to the fall in the labor force participation of men, and stagnation in the labor force participation rate of women, in particular during 1994⁠–2020.
...
```{r message=FALSE, warning=FALSE}
load("dmonthly.RData")
dmonthly$labor_force_nonparticipation_rate_men = 1 - dmonthly$lfp_rate_men
p <-
  ggplot(dmonthly, aes(time))  +
  
  geom_line(
    aes(y = labor_force_nonparticipation_rate_men, color = "Labor_Force_Nonparticipation_Rate")
  ) +
...
tab <-
  dmonthly  %>% mutate(
    ...
    month_num = month,
    month = month.abb[month],
    labor_force_nonparticipation_rate_men=round(as.numeric(labor_force_nonparticipation_rate_men), 3),
    housework_rate_men=round(as.numeric(housework_rate_men), 3),
      ...
      other_reason_rate_men=round(as.numeric(other_reason_rate_men), 3)) %>% select(c(year, month_num, month, labor_force_nonparticipation_rate_men,housework_rate_men,disability_rate_men,retired_rate_men,school_rate_men,other_reason_rate_men))
...
DT::datatable(
  ...
  options = list(dom = 'Blfrtip',
                 buttons = c('copy', 'csv', 'excel'))
) %>% formatPercentage(c("labor_force_nonparticipation_rate_men","housework_rate_men","disability_rate_men","retired_rate_men","school_rate_men","other_reason_rate_men"), 1)
...
```{r message=FALSE, warning=FALSE}
dmonthly$labor_force_nonparticipation_rate_women = 1 - dmonthly$lfp_rate_women
p <-
  ggplot(dmonthly, aes(time))  +
  
  geom_line(
    aes(y = labor_force_nonparticipation_rate_women, color = "Labor_Force_Nonparticipation_Rate")
  ) +
...
#### Table 3. Working-Age Women Not in the Labor Force by Reported Reasons, 1976–2020
...
tab <-
  dmonthly  %>% mutate(
    ...
    month_num = month,
    month = month.abb[month],
    labor_force_nonparticipation_rate_women=round(as.numeric(labor_force_nonparticipation_rate_women), 3),
    housework_rate_women=round(as.numeric(housework_rate_women), 3),
      ...
      other_reason_rate_women=round(as.numeric(other_reason_rate_women), 3)) %>% select(c(year, month_num, month, labor_force_nonparticipation_rate_women,housework_rate_women,disability_rate_women,retired_rate_women,school_rate_women,other_reason_rate_women))
...
DT::datatable(
  ...
  options = list(dom = 'Blfrtip',
                 buttons = c('copy', 'csv', 'excel'))
) %>% formatPercentage(c("labor_force_nonparticipation_rate_women","housework_rate_women","disability_rate_women","retired_rate_women","school_rate_women","other_reason_rate_women"), 1)
...
Figure 5 shows the detrended seasonal component (red) and the combined seasonal and irregular component (blue) of the male labor force participation rate. The straight lines are the average seasonal components over the whole sample period. Labor force participation peaks in summer, around July, and is smallest in winter, in particular, in January. The summer peak became weaker during 1976–2020, and the winter trough increased.
